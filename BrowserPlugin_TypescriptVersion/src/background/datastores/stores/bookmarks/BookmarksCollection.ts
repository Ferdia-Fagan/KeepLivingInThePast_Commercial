...
class BookmarksCollectionBuildingManager implements BuildingSetupCheckInterface{
    ...
    static tagsCollectionBuilder = builder<BookmarkObject, string, BookmarkObjectUpdateReport,
        BookmarksCollection>(
        "WebpageTags", 1, "TagsCollection",
        BookmarksCollectionBuildingManager.tagsCollectionDatabaseAndTableSetup,
        BookmarksCollection
    ).then(tagsCollectionInstance =>
        bookmarksCollection = tagsCollectionInstance
    )
